      ...
      >
        ...
        <div className="p-4 sm:p-5 border-b border-[#E8DFC8] bg-white flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-xl bg-[#183928] text-[#E6C687] flex items-center justify-center">
        ...
        {cart.length > 0 && (
          <div className="px-4 py-2.5 bg-[#F4ECE1] border-b border-[#E8DFC8]">
            <div className="flex items-center justify-between text-xs font-semibold mb-1.5">
              <span className="text-[#8F4A2D] flex items-center gap-1">
              ...
              <span className="text-stone-500 text-[11px]">{freeDeliveryProgress}%</span>
            </div>
            <div className="w-full bg-[#E5D7C5] h-1.5 rounded-full overflow-hidden">
              <div
                className="bg-[#C85A32] h-full rounded-full transition-all duration-300"
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4 custom-scrollbar">
          {cart.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center py-12 px-4 space-y-4">
              <div className="w-20 h-20 rounded-full bg-[#F3ECE0] flex items-center justify-center text-4xl shadow-inner">
                🍽️
              </div>
              ...
              <button
                ...
                className="px-6 py-2.5 rounded-xl bg-[#183928] text-white hover:bg-[#11271B] text-xs font-bold shadow-md transition-all active:scale-95"
              >
                Explore Menu
              </button>
            </div>
          ) : (
            <div className="divide-y divide-[#EBE3D5] space-y-3">
              {cart.map((item) => (
                <div key={item.id} className="pt-3 first:pt-0 flex gap-3 group">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      <div className="flex items-center bg-[#F1EAE0] rounded-lg p-0.5">
                        <button
                          onClick={() => updateCartItemQuantity(item.id, -1)}
                          className="w-6 h-6 rounded flex items-center justify-center text-stone-700 hover:bg-white transition-colors"
                        >
                      ...
                      <span className="text-sm font-bold text-[#183928]">
                        ₹{item.total_price}
                      </span>
                    </div>
                  </div>
              ...
              {appliedCoupon ? (
                ...
                <form onSubmit={handleApplyCoupon} className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-3 py-2 rounded-xl border border-stone-300 bg-white text-xs uppercase placeholder:normal-case font-semibold text-stone-800 focus:outline-none focus:border-[#183928]"
                  />
                  <button
                    type="submit"
                    className="px-4 py-2 bg-[#183928] text-white rounded-xl text-xs font-bold hover:bg-[#10271B] transition-colors"
                  >
                    Apply
                  </button>
                </form>
        ...
        {cart.length > 0 && (
          <div className="p-4 sm:p-5 bg-white border-t border-[#E8DFC8] space-y-3 shadow-lg">
            {/* Price Calculations */}
            <div className="space-y-1.5 text-xs text-stone-600">
              ...
              <div className="pt-2 border-t border-stone-200 flex justify-between text-base font-extrabold text-[#183928]">
            ...
            <div className="space-y-2 pt-1">
              <button
                ...
                className="w-full py-3.5 px-4 rounded-xl bg-[#25D366] text-stone-900 hover:bg-[#1EBE5D] font-extrabold text-sm flex items-center justify-center gap-2 shadow-md active:scale-98 transition-all"
              >
