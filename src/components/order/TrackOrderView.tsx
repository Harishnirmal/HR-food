  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 space-y-8">
      ...
      <div className="text-center max-w-xl mx-auto space-y-2">
        ...
        <h1 className="font-serif text-3xl sm:text-4xl font-bold text-[#941B1B]">
      ...
      <form onSubmit={handleSearch} className="max-w-md mx-auto flex gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 rounded-2xl border border-[#F2DDD0] bg-white text-sm text-[#231815] font-semibold focus:outline-none focus:border-[#EA580C] shadow-xs"
          />
        </div>
        <button
          type="submit"
          className="px-6 py-3 rounded-2xl bg-gradient-to-r from-[#941B1B] to-[#EA580C] text-white hover:opacity-90 font-bold text-sm shadow-md transition-all active:scale-95 shrink-0 cursor-pointer"
        >
          Track
        </button>
      </form>
      ...
      {searchedOrder && (
        <div className="bg-white rounded-3xl border border-[#F2DDD0] p-6 sm:p-8 shadow-md space-y-6 animate-in fade-in duration-200">
          
          {/* Header */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-stone-100">
            <div>
              <div className="flex items-center gap-3">
                <h2 className="font-serif text-xl sm:text-2xl font-bold text-[#231815]">
                  Order #{searchedOrder.order_number}
                </h2>
                <OrderStatusBadge status={searchedOrder.status} />
              </div>
              <p className="text-xs text-[#826A62] mt-1">
                Placed for <span className="font-bold text-[#231815]">{searchedOrder.customer_name}</span> ({searchedOrder.phone})
              </p>
            </div>

            <div className="text-right">
              <span className="text-xs text-[#826A62]">Total Bill</span>
              <p className="font-serif font-bold text-2xl text-[#941B1B]">₹{searchedOrder.total}</p>
            </div>
          </div>

          {/* Timeline Progress */}
          <div className="bg-[#FCF8F5] p-5 rounded-2xl border border-[#F2DDD0] space-y-2">
            <h4 className="text-xs font-bold uppercase tracking-wider text-[#826A62] mb-1">
          ...
          <div className="p-4 rounded-2xl bg-[#EAF7EE] border border-[#25D366]/40 flex flex-col sm:flex-row items-center justify-between gap-3">
            ...
            <a
              ...
              className="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-[#25D366] text-white font-extrabold text-xs hover:bg-[#1EBE5D] flex items-center justify-center gap-1.5 shadow-xs transition-all active:scale-95 shrink-0"
            >
            ...
            <div className="divide-y divide-stone-100 text-xs sm:text-sm space-y-2">
              {searchedOrder.items.map((item: any) => (
                <div key={item.id} className="pt-2 first:pt-0 flex justify-between">
                  <div>
                    <span className="font-bold text-[#231815]">{item.product_name}</span> × {item.quantity}
                    {item.customizations?.addons && item.customizations.addons.length > 0 && (
                      <p className="text-[11px] text-[#826A62]">
          ...
          <div className="p-4 bg-[#FCF8F5] rounded-2xl border border-[#F2DDD0] text-xs text-[#6E564F] space-y-1">
          ...
          <div className="pt-2 flex justify-end">
            <button
              ...
              className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-[#941B1B] to-[#EA580C] text-white hover:opacity-90 font-bold text-xs flex items-center gap-1.5 shadow-md active:scale-95 transition-all cursor-pointer"
            >
